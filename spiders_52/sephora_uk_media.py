...
class SephoraUkMediaSpider(scrapy.Spider):
    ...
    def spider_closed(self, spider):
        ...
        if os.path.exists(self.screenshot_folder):
            ...
            if os.path.exists(screenshot_zip_file):
                os.system(f"/usr/local/bin/aws s3 cp {screenshot_zip_file}  s3://scraping-external-feeds-lapis-data/{self.CURRENT_DATE}/sephora_uk/")
                logging.warning(f"Completed aws s3 cp {screenshot_zip_file}  s3://scraping-external-feeds-lapis-data/{self.CURRENT_DATE}/sephora_uk/")
    ...
    async def sponsored_listing_page(self, media_search_url,item):
        try:
            ...
            self.page.wait_for_timeout(3000)
            try:
                media_search_accepct_selector = Selector(text=self.page.content())
                if media_search_accepct_selector.xpath('//div[@id="notice-content"]/div[@class="right"]/a'):
                    self.page.locator('//div[@id="notice-content"]/div[@class="right"]/a').click()
                if media_search_accepct_selector.xpath('//div[@class="dy-modal-contents"]/div[@class="dy-lb-close"]'):
                    self.page.locator('//div[@class="dy-modal-contents"]/div[@class="dy-lb-close"]').click()
            except:
                logging.warning("ERROR AT LINE 140")
            ...
            for sponsored_listing in media_search_selector.xpath('//div[@class="Product Product- sponsored"]'):
                ...
                data['brand'] = None
                if re.search(r'<script\s*type\=\"application\/ld\+json\"[^>]*?>[\w\W]*?\"brand\"\:\s*\"([^>]*?)\"[^>]*?<\/script>',product_response.text):
                    data['brand'] = re.findall(r'<script\s*type\=\"application\/ld\+json\"[^>]*?>[\w\W]*?\"brand\"\:\s*\"([^>]*?)\"[^>]*?<\/script>',product_response.text)[0] 
    ...
    async def parse(self, response):
        ...
        for media_search_url in media_search_urls:
            ...
            item =  await self.sponsored_listing_page(media_search_url.strip(),item)
    ...
    async def homepage_product_listing(self, response,item):
        ...
        product_links = response.xpath('//div[@class="eba-component eba-product-listing"]/div[contains(@class,"Product")]/a/@href|//div[@class="Product"]/a/@href|//div[contains(@class,"ProductPanel chanel chanel-panel")]/a/@href').getall()
        
        if item['products_count'] == 0:
            item['products_count'] = int(products_count) if products_count else len(product_links)
        if item['products_count'] == 0:
            if response.xpath('//h1/span[@class="pdp-product-brand-name"]/text()').get('').strip():
                product_links.append(response.url)
        ...
        for product_link in product_links:
            try:
                ...
                if  re.search('\"data\"\:\[\"([^>]*?)\"\]',product_response.text):
                    data["product_id"] = re.findall('\"data\"\:\[\"([^>]*?)\"\]',product_response.text)[0]
                if product_response.xpath('//h1/span[@class="pdp-product-brand-name"]/text()|//h1[@itemprop="name"]/span//text()').get('').strip():
                    data["name"] =  product_response.xpath('//h1/span[@class="pdp-product-brand-name"]/text()').get('').strip()
                    if data['name'] == "":
                        data["name"] =  (' '.join(product_response.xpath('//h1[@itemprop="name"]/span//text()').getall())).strip()
                    data['brand'] = None
                    if re.search(r'<script\s*type\=\"application\/ld\+json\"[^>]*?>[\w\W]*?\"brand\"\:\s*\"([^>]*?)\"[^>]*?<\/script>',product_response.text):
                        data['brand'] = re.findall(r'<script\s*type\=\"application\/ld\+json\"[^>]*?>[\w\W]*?\"brand\"\:\s*\"([^>]*?)\"[^>]*?<\/script>',product_response.text)[0] 
