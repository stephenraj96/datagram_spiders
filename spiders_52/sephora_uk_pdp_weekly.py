...
class SephoraUKPdpSpider(scrapy.Spider):
    ...
    def parse(self, response):
        spider_name = SephoraUKPdpSpider.name.replace('pdp_weekly','categories')
    ...
    async def product_detail(self,response,category_name,category_url,main_category,main_category_url,sub_category,sub_cat_url):
        ...
        if response.xpath('//h1/span[@class="pdp-product-brand-name"]/text()').get('').strip():
            ...
            image_urls = [ url.replace(' ','%20') for url in image_urls if not 'player.' in url ]
            video_url = [ url for url in image_url_list if 'youtube' in url ]
            video_url_player = [ url for url in image_url_list if 'player.' in url ]
            if video_url_player:
            ...
            item['brand'] = None
            if re.search(r'<script\s*type\=\"application\/ld\+json\"[^>]*?>[\w\W]*?\"brand\"\:\s*\"([^>]*?)\"[^>]*?<\/script>',response.text):
                item['brand'] = re.findall(r'<script\s*type\=\"application\/ld\+json\"[^>]*?>[\w\W]*?\"brand\"\:\s*\"([^>]*?)\"[^>]*?<\/script>',response.text)[0] 
            ...
            pass_key_url = 'https://display.ugc.bazaarvoice.com/static/sephora-uk/main_site/en_GB/bvapi.js'    
            pass_key_response = await self.requests_process(pass_key_url)
            pass_key,display_code='',''
            if re.search(r'passkey\:\"([^>]*?)\"',pass_key_response.text):
                pass_key = re.findall(r'passkey\:\"([^>]*?)\"',pass_key_response.text)[0]
            if re.search(r'displaycode\:\"([^>]*?)\"',pass_key_response.text):
                display_code = re.findall(r'displaycode\:\"([^>]*?)\"',pass_key_response.text)[0]
            
            review_url = f'https://api.bazaarvoice.com/data/batch.json?passkey={pass_key}&apiversion=5.5&displaycode={display_code}&resource.q0=products&filter.q0=id%3Aeq%3A{item["master_product_id"]}&stats.q0=reviews&filteredstats.q0=reviews&filter_reviews.q0=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&filter_reviewcomments.q0=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&resource.q1=reviews&filter.q1=isratingsonly%3Aeq%3Afalse&filter.q1=productid%3Aeq%3A{item["master_product_id"]}&filter.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&sort.q1=relevancy%3Aa1&stats.q1=reviews&filteredstats.q1=reviews&include.q1=authors%2Cproducts%2Ccomments&filter_reviews.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&filter_reviewcomments.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&filter_comments.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&limit.q1=100&offset.q1=0&limit_comments.q1=3&callback=BV._internal.dataHandler0'
            review_response = await self.requests_process(review_url)
            review_list = []
            if re.search(r'BV\._internal\.dataHandler0\(([^>]*?)\)$',review_response.text):
                review_block = re.findall(r'BV\._internal\.dataHandler0\(([^>]*?)\)$',review_response.text)[0]
                review_json = json.loads(review_block)
                review_list = self.review(review_json,review_list)
                count = review_json.get('BatchedResults').get('q1').get('TotalResults')
                count = count if count else 0
                for page_count in range(1,round(int(count)/100)+1):
                    review_next_page_url = f'https://api.bazaarvoice.com/data/batch.json?passkey={pass_key}&apiversion=5.5&displaycode={display_code}&resource.q0=products&filter.q0=id%3Aeq%3A{item["master_product_id"]}&stats.q0=reviews&filteredstats.q0=reviews&filter_reviews.q0=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&filter_reviewcomments.q0=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&resource.q1=reviews&filter.q1=isratingsonly%3Aeq%3Afalse&filter.q1=productid%3Aeq%3A{item["master_product_id"]}&filter.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&sort.q1=relevancy%3Aa1&stats.q1=reviews&filteredstats.q1=reviews&include.q1=authors%2Cproducts%2Ccomments&filter_reviews.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&filter_reviewcomments.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&filter_comments.q1=contentlocale%3Aeq%3Aen*%2Cfr*%2Cen_GB&limit.q1=100&offset.q1={100*page_count}&limit_comments.q1=3&callback=BV._internal.dataHandler0'
                    next_page_review_response = await self.requests_process(review_next_page_url)
                    if re.search(r'BV\._internal\.dataHandler0\(([^>]*?)\)$',next_page_review_response.text):
                        next_review_block = re.findall(r'BV\._internal\.dataHandler0\(([^>]*?)\)$',next_page_review_response.text)[0]
                        next_review_json = json.loads(next_review_block)
                        review_list = self.review(next_review_json,review_list)
                    
            item["reviews"] = review_list
            yield item
    
    def review(self,response,review_list):
        for review_values in response.get('BatchedResults',{}).get('q1',{}).get('Results'):
            review_dict = {}
            review_dict['user'] = review_values.get('UserNickname')
            review_dict['date'] = review_values.get('SubmissionTime','').split('T')[0]
            review_dict['stars'] = review_values.get('Rating')
            review_title = review_values.get('Title')
            review_body = review_values.get('ReviewText')
