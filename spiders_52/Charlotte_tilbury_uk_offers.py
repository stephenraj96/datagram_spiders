...
class CharlotteTilburyUkOffersSpider(scrapy.Spider):
    name = 'Charlotte_tilbury_uk_offers'
    ...
    def parse(self, response):
        ...
        with open(f'{supporting_files}/Charlotte_tilbury_uk_input_urls.txt','r') as f:
            urls=f.readlines()
        for url in urls:
    ...
    async def parse_category(self,response):
        ...
        item['count']=None
        if '/uk?search=' in response.url:#search pages the format of getting value is differ from product url
        ...
        else: # we are getting value directly from the source page using regex.
        ...
        for i in categories:
            ...
            if response.url.__contains__('/products/'):
                category['url']=f'https://www.charlottetilbury.com/uk/products/{i["href"]}'
            item['category_crumb'].append(category)
        item['products']=[]
        for count,product in enumerate(data,start=1): #if inner categories is obtained
            product_url=f'https://www.charlottetilbury.com/uk/product/{product["href"]}'
                ...
                if product_response.status==200:
                    ...
                    if sibling:
                        for siblings in product_json_data['props']['siblings']:
                            products={}
                            products["rank"]= count
                            products["url"]=f'https://www.charlottetilbury.com/uk/product/{siblings["href"]}'
                            products["image_url"]=[f"https:{i['imageSrc']}" for i in siblings['images']] 
                            products['has_video']=False
                            products['video']=[]
                            
                            video=siblings.get('shortVideo','')
                            ...
                            products["master_product_id"]= siblings["sku"]
                            products["name"]= siblings['title']
                            subtitle=siblings.get('subtitle','')
                            if subtitle:
                                products["name"]= f"{siblings['title']} - {subtitle}"
                            ...
                            if price_before:
                                products["price_before"]= f'{"%.2f" %price_before} £'
                            products["promo_label"]= None
                            promo_lable=siblings.get("badge",'')
                            if promo_lable:
                        ...
                        products["url"]=f'https://www.charlottetilbury.com/uk/product/{product_json_data["props"]["product"]["href"]}'
