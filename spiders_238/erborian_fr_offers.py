...
class ErborianFrOffersSpider(scrapy.Spider):
    ...
    def product_details(self, url, rank, block):
        ...
        if re.search(r"id\:\s*\'(.*?)'\,\s*sku", url.text):
                item_2["master_product_id"] = re.findall(
                    r"id\:\s*\'(.*?)'\,\s*sku", url.text
                )[0]
        else:
        ...
        stock = (
            product.xpath('//div[@class="m-add-to-cart-static"]/button/text()')
            .get("")
            .strip()
        )
        if "Ajouter" in stock:
        ...
        if product.xpath(
            ...
            price_before = "".join(
                product.xpath('//div[@class="m-product-price"]/span[2]/text()|//*[contains(text(),"Ancien prix:")]/parent::p/text()').getall()
            ).strip()
        ...
        if product.xpath('//p[@class="a-price-sales"]/text()'):
            item_2["price"] = (
                product.xpath('//p[@class="a-price-sales"]/text()').get("").strip()
            )
        else:
            item_2["price"] = None
    ...
    def start_requests(self):
        ...
        for offer_url in offer_urls:
            url = f"https://api.scrape.do/?token={self.api_token}&url={offer_url}"
            time.sleep(5)
    ...
    async def parse(self, response):
        ...
        block = ""
        count_value = response.xpath(
            '//h1[@aria-live="assertive"]//text()|//h1/span[2]//text()'
        ).get("")
        if response.xpath("//h1"):
            ...
            if "recherche?" in response.url:
                if count_value:
                    ...
                    item["title"] = (
                        response.xpath("//h1/strong/text()").get("").replace('"', "")
                    )
                    item["count"] = 0
                    item["products"] = []
            else:
            ...
            if "Aucun résultat" not in response.xpath("//h1/text()").get("").strip():
                ...
                product_list.append(product_link_collection)
                two_catgory_search = [
                    "?srule=Category-default&sz=12&start=12&format=ajax&type=view-more&context=products-list",
                    "&srule=Name&sz=12&start=12&format=ajax&type=view-more&context=products-list",
                ]
                for pages_cat in range(1, round(item["count"] / 12) + 1):
                    if "recherche" not in link:
                        ...
                        next_page_url_cat = await self.requests_process(
                            next_page_link, headers=self.headers
                        )
                        time.sleep(3)
                        if re.search(
                            "Please\s*enable\s*JS\s*and\s*disable\s*any\s*ad\s*blocker",
                            next_page_url_cat.text,
                        ):
                            time.sleep(4)
                            (
                                next_page_url_cat,
                                next_page_url_cat_re,
                            ) = self.retry_function_product_coll(next_page_link)
                    else:
                        urls = f"{link}{two_catgory_search[1]}"
                        page_change = urls.replace(
                            "sz=12&start=12", f"sz=12&start={str(pages_cat * 12)}"
                        )
                        next_page_link = f"https://api.scrape.do/?token={self.api_token}&url={urllib.parse.quote(page_change)}"
                    ...
                    product_link_collection_cat = next_page_url_cat.xpath(
                        '//h2[@class="m-product-name"]/a/@href'
                    ).getall()
                ...
                for rank, block in enumerate(product_list_collection, 1):
                    if block:
                        ...
                        product_link = f"https://api.scrape.do/?token={self.api_token}&url={urllib.parse.quote(block)}"
                        url = await self.requests_process(
                            product_link, headers=self.headers
                        )
                        if re.search(
                            "Please\s*enable\s*JS\s*and\s*disable\s*any\s*ad\s*blocker",
                            url.text,
